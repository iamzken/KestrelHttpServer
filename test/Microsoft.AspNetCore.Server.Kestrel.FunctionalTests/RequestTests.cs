...
namespace Microsoft.AspNetCore.Server.Kestrel.FunctionalTests
{
    ...
    {
        const int semaphoreWaitTimeout = 2500;
        ...
        public async Task ConnectionResetAbortsRequest()
        {
            ...
            using (var host = builder.Build())
            {
                ...
                using (var socket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp))
                {
                    ...
                    Assert.True(await connectionStarted.WaitAsync(semaphoreWaitTimeout));
                    // Force a reset
                    socket.LingerState = new LingerOption(true, 0);
                }

                // Wait until connection error is logged
                Assert.True(await connectionErrorLogged.WaitAsync(semaphoreWaitTimeout));
        ...
        public async Task ThrowsOnReadAfterConnectionError()
        {
            ...
            var builder = new WebHostBuilder()
                ...
                .Configure(app => app.Run(async context =>
                {
                    ...
                    Assert.True(await connectionReset.WaitAsync(semaphoreWaitTimeout));
            ...
            using (var host = builder.Build())
            {
                ...
                using (var socket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp))
                {
                    ...
                    Assert.True(await requestStarted.WaitAsync(semaphoreWaitTimeout));
                }

                connectionReset.Release();

                Assert.True(await appDone.WaitAsync(semaphoreWaitTimeout));
